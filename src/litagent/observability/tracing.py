"""Translate LitAgent lifecycle events into LangFuse observations."""
...
class LangFuseTracer:
        ...
        if event == "survey.start":
            ...
            if span:
                if event == "worker.failed":
                    ...
                    span.update(
                        level="WARNING", status_message=data.get("error", "cancelled")
                    )
                elif "output" in data:
    ...
    def _handle_io_event(self, event: str, data: dict[str, Any]) -> None:
        ...
        if phase == "start":
            ...
            self._operations[op_id] = parent.start_observation(
                ...
                input={
                    ...
                    if k not in ("operation_id", "task_id")
                },
            )
        ...
        if phase == "failed":
            # Stable codes avoid forwarding raw exception text to external traces.
    ...
    def _safe_update_output(self, span, output) -> None:
        """Attach output with a serializable diagnostic fallback."""
        try:
    ...
    def _close_orphans(self) -> None:
        ...
        for op_id, obs in list(self._operations.items()):
            try:
                obs.update(
                    level="WARNING",
                    status_message="Orphan operation (closed by cleanup)",
                )
                obs.end()
            except Exception:
                pass
    ...
    def flush(self) -> None:
        """Flush pending observations when tracing is enabled."""
