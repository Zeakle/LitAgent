"""Reviewer Worker — evidence-compliance adversarial review."""
...
from litagent.context.evidence_selector import EvidenceSelection, format_evidence_selection
...
REVIEWER_INSTRUCTIONS = """Review the survey draft with rigorous evidence-compliance standards.
...
Respond in JSON format:
{
    ...
    "evidence_compliance": {
        "passed": true|false,
        "unsupported_claims": [
            {"section": "...", "claim": "...", "reason": "...", "action": "delete|downgrade|bind"}
        ],
...
_REQUIRED_REVIEW_KEYS = frozenset({
    "score", "strengths", "weaknesses", "issues",
...
class ReviewerWorker(Worker):
    ...
    Input: survey draft + evidence_selection
    Output: structured review JSON with evidence_compliance
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._llm = llm
        self._claims_index = claims_index    # retained, not deleted — Phase 14 will restore
    ...
    async def execute(self, task: SubTask) -> Any:
        ...
        skills_text = (
            self._skill_manager.to_metadata_text_for(
                "reviewing a literature survey draft", top_k=2,
            )
        ...
        review = self._enforce_evidence_contract(
            draft, selection,
            review,
            evidence_context_missing=evidence_missing
        )
        ...
        logger.info(
            ...
            f"evidence_passed: {review.get('evidence_compliance', {}).get('passed', 'N/A')}"
        )
        return review
    ...
    async def review_revision(
        ...
        """Review a revision using the SAME evidence_selection."""
    ...
    async def _build_review_context(
        ...
        """Build reviewer context pipeline (draft priority 0, evidence priority 1).

        Same builder for both first review and revision — guarantees consistent
        budget, XML structure, and evidence format.
        """
    ...
    def _enforce_evidence_contract(
        ...
        *,
        ...
        """Deterministic evidence compliance check — do NOT trust model self-report.

        Fail-closed: any unknown ID, unsupported claim, schema error, or missing
        selection → passed=False, score clamped, accept->revise.
        """
        ...
        required_compliance_keys = {
            'passed', 'unsupported_claims', 'unknown_evidence_ids'
        }
        ...
        # Model-reported passed — must be bool
        ...
        logger.info(
            ...
            passed, len(valid_ids), len(draft_refs), len(unknown_from_draft),
            len(model_unknown), len(unsupported), schema_invalid, evidence_context_missing,
        )
        return review
    ...
    def _parse_review(self, content: str) -> dict[str, Any]:
        try:
            ...
            # Fail-closed: missing/malformed evidence_compliance → passed=False
    ...
    def _extract_key_phrases(self, draft: str) -> list[str]:
        ...
        return [
            ...
            if any(kw in s.lower() for kw in [
                "achieve", "%", "outperform", "state-of-the-art", "sota",
            ])
        ][:10]
