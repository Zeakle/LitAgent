"""Evidence Selector — per-section CrossEncoder ranking + round-robin packing.

Reuses the shared CrossEncoder Reranker from composition root.
Does NOT load models; reranker=None is a legal degraded state.
"""
...
class SectionSpec:
    """Immutable section definition. key maps to Synthesis output structure."""
    key: str
    title: str
    retrieval_hint: str
...
DEFAULT_SECTIONS = (
    SectionSpec("introduction", "Introduction and Motivation",
                "background context, problem statement, and motivation for the survey topic"),
    SectionSpec("taxonomy", "Taxonomy of Approaches",
                "categorization of methods, approaches, architectures, and techniques"),
    SectionSpec("methods", "Detailed Analysis of Key Methods",
                "specific algorithms, model architectures, training procedures, technical details"),
    SectionSpec("experiments", "Experimental Comparison",
                "benchmarks, datasets, metrics, quantitative results, performance numbers"),
    SectionSpec("open_problems", "Open Problems and Future Directions",
...
class EvidenceSelection:
    """Result of one evidence selection pass. Serializable via to_dict/from_dict."""
    candidate_count: int
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "EvidenceSelection":
        """Validate and reconstruct from a JSON-safe dict.

        Raises ValueError on malformed input — never silently returns an
        empty selection for a non-empty dict.
        """
        if not isinstance(value, Mapping):
            raise ValueError(f"EvidenceSelection.from_dict expects Mapping, got {type(value).__name__}")
        ...
        if not isinstance(raw_items, Mapping):
            raise ValueError(f"selected_items must be Mapping, got {type(raw_items).__name__}")
        ...
        for k, v in raw_items.items():
            ...
            if not isinstance(v, Mapping):
                raise ValueError(f"selected_items[{k!r}] must be Mapping, got {type(v).__name__}")

            item = dict(v)
            if item.get("evidence_id") != k:
                raise ValueError(f"selected_items[{k!r}].evidence_id must equal its key")
        ...
        if not isinstance(raw_sec, Mapping):
            raise ValueError(f"section_evidence_ids must be Mapping, got {type(raw_sec).__name__}")
        ...
        for k, v in raw_sec.items():
            ...
            if not isinstance(v, list):
                raise ValueError(f"section_evidence_ids[{k!r}] must be list, got {type(v).__name__}")

            ids: list[str] = []
            for evidence_id in v:
                if not isinstance(evidence_id, str) or evidence_id not in selected_items:
                    raise ValueError(
                        f"section_evidence_ids[{k!r}] contains unknown id {evidence_id!r}"
                    )
                if evidence_id in ids:
                    raise ValueError(
                        f"section_evidence_ids[{k!r}] contains duplicate id {evidence_id!r}"
                    )
        ...
        if not isinstance(raw_method, Mapping):
            raise ValueError(f"method_by_section must be Mapping, got {type(raw_method).__name__}")
        ...
        if cand_count < len(selected_items):
            raise ValueError("candidate_count cannot be smaller than selected item count")

        if oc != cand_count - len(selected_items):
            raise ValueError("omitted_count must equal candidate_count - selected item count")
...
class EvidenceSelector:
    """Per-section evidence retrieval with round-robin packing.

    Created once in Runner._wire() using Infra.reranker.
    Never constructs its own CrossEncoder.
    """
    ...
    async def select(
        ...
        *,
        ...
        """Rank and pack evidence from ledger.

        Lifecycle events: evidence.retrieve.start → complete/failed
        CancelledError → evidence.retrieve.failed(reason_code="cancelled") + re-raise.
        """
        if max_tokens <= 0:
        ...
        self._emit("evidence.retrieve.start", {
        ...
        try:
            ...
            self._emit("evidence.retrieve.failed", {
                "operation_id": operation_id,
                "task_id": task_id,
        ...
        elapsed = int((time.perf_counter() - t0) * 1000)
        self._emit("evidence.retrieve.complete", {
            "operation_id": operation_id,
            "task_id": task_id,
            ...
            if self._reranker is not None and query:
                try:
                    ...
                    ranked = await asyncio.to_thread(
                        self._reranker.rerank, section_query, docs,
                    )

                    # Validate results: must have Doc + known evidence_id + non-duplicate
                    ...
                    for scored_doc in ranked:
                        ...
                        if not isinstance(scored_doc.doc, Document):
                            raise ValueError("reranker returned ScoredDoc without Document")
                        ...
                        if eid in seen:
                            raise ValueError('reranker returned a duplicate evidence id')
    ...
    def _lexical_rank_section(
        ...
        """Token coverage score + original ledger order as tie-breaker"""
...
def format_evidence_selection(selection: EvidenceSelection) -> str:
    """Format EvidenceSelection as <evidence_plan> + <evidence_ledger> XML.

    Uses html.escape(quote=True) on section keys, titles, and evidence text
    to prevent paper content from closing XML tags or injecting pseudo-instructions.

    Empty selection returns complete empty tags. No max_chars, no "... omitted".
    """
