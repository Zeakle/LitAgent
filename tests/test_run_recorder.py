"""Tests for durable run recording and archive replay."""
...
def test_recorder_pairs_lifecycle_events_and_persists_full_payload(tmp_path):
    ...
    assert (
        json.loads(repository.path_for("run-1").read_text(encoding="utf-8"))["run_id"]
        == "run-1"
    )
...
def test_recorder_maps_rag_memory_and_usage_payloads(tmp_path):
    ...
    assert (
        artifact["nodes"]["rag.search:rag-1"]["output"]["results"][0]["title"]
        == "Paper"
    )
...
def test_repository_reads_legacy_v1_archive_without_rewriting(tmp_path):
    ...
    assert (
        json.loads(repository.path_for("legacy").read_text(encoding="utf-8")) == legacy
    )
